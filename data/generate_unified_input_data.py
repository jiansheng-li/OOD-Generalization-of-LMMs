...
def gen_sample_json(dataset='PACS', args=None):
    ...
    for domain in domains:
        ...
        if os.path.exists(domain_path) and os.path.isdir(domain_path):
            for class_id, class_name in enumerate(selected_images_info['class_names']):
                ...
                if os.path.exists(class_path) and os.path.isdir(class_path):
                    ...
                    if len(images) >= args.num_sample:
                        sampled_images = images[:args.num_sample]
                    else:
            ...
            logger.info(f"The domain {domain} does not exist.")

    with open(os.path.join(args.output_dir, f'unified_input_{dataset}.json'), 'w') as f:
        json.dump(selected_images_info, f, indent=4)


def convert_unified_input_into_zeroshot_vqa(dataset, data, args):
    question_file = os.path.join(args.output_dir, f'input_{dataset}_in_{args.model_name}_vqa.jsonl')
    first_flag = True
    ...
    for item_id, item in data['samples'].items():
        ...
        prompt = (
            f"Given the image, answer the following question using the specified format.\n"
            f"Question: What is in this image? Choice list: {class_names}.\n"
            f"Please choose a choice from the list and respond with the following format:\n"
            f"---BEGIN FORMAT TEMPLATE---\n"
            f"Answer Choice: [Your Answer Choice Here]\n"
            f"---END FORMAT TEMPLATE---\n"
            f"Do not deviate from the above format. Repeat the format template for the answer."
        )
